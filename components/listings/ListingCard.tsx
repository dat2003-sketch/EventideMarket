...
const ACCENT = '#45b672ff';

export default function ListingCard({ item, onPress, onEdit, onDelete, editable }: any) {
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.9}>
      ...
      <View style={{ padding: 10 }}>
        ...
        <Text style={{ color: ACCENT, fontSize: 16 }}>${Number(item.price).toFixed(0)}</Text>
        {editable && (
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 6 }}>
            {!!onEdit && <Text onPress={onEdit} style={{ color: '#2563eb' }}>Edit</Text>}
            {!!onDelete && <Text onPress={onDelete} style={{ color: '#dc2626' }}>Delete</Text>}
          </View>
        )}
      </View>
    </TouchableOpacity>
